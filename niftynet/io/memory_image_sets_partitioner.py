...
class MemoryImageSetsPartitioner(BaseImageSetsPartitioner):
    ...
    def initialise(self,
        ...
        self._callback_functions = data_param.input_callback_dicts
    ...
    def get_image_lists_by(self, phase=None, action='train'):
        ...
        if phase:
            if phase == ALL:
                images = [idcs for idcs in self._partitions.values()]
            else:
                images = self._partitions[phase]
        elif action and TRAIN.startswith(action):
            images = self._partitions[TRAIN] + self._partitions[VALID]
        else:
            images = self._partitions[INFER]
